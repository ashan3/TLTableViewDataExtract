...
{
    return [NSString stringWithFormat:@"%lu-%02lu-%02lu %02lu:%02lu",(unsigned long)[date year],(unsigned long)[date month], [date day], [date hour], [date minute]];
}
...
- (NSString *) dayInfo
{
    ...
    else if ([self isSameDay:[[NSDate date] offsetDays:-1]]) {
        return @"昨天";
    }
    else if ([self isSameDay:[[NSDate date] offsetDays:-2]]) {
        return @"前天";
    }
    else {
        return [NSString stringWithFormat:@"%ld-%ld", self.month, self.day];
    }
}
@end
